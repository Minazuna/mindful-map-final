...
import { generateSectionStatisticsPDF } from '../PDFTemplates/SectionStatisticsPDF';
...
function getSummaryPhrase(title, data) {
  ...
  if (title === 'Sleep') {
    return `Most students logged "${beautifyName(top.activity)}" hours of sleep most often (${top.count} times, ${top.percent}%). Getting enough sleep is important for mood and focus!`;
  }
  if (top.percent >= 50) {
...
const SectionStatistics = () => {
  ...
  const { emotion, moodType, moodPeriod, section } = location.state || {};
  ...
  useEffect(() => {
    if (!emotion || !moodType || !moodPeriod || !section) {
      navigate('/teacher/mood-analysis');
      return;
    }
    setLoading(true);
    fetch(
      `${import.meta.env.VITE_NODE_API}/api/teacher/section-mood-activities?section=${encodeURIComponent(section)}&emotion=${encodeURIComponent(emotion)}&moodType=${moodType}&period=${moodPeriod}`,
      {
        headers: {
  ...
  }, [emotion, moodType, moodPeriod, section, navigate]);
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      const sectionsData = [
        { title: 'Activity', data: grouped.activity, isSleepHours: false },
        { title: 'Social', data: grouped.social, isSleepHours: false },
        { title: 'Health', data: grouped.health, isSleepHours: false },
        { title: 'Sleep', data: grouped.sleep, isSleepHours: true }
      ];
      await generateSectionStatisticsPDF(section, emotion, moodType, moodPeriod, sectionsData);
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-[#f7fafc] via-[#e0f7fa] to-[#e6f4ea] overflow-x-hidden">
      <TeacherSidebar teacher={teacher} />
      <div className="flex-1 ml-[var(--sidebar-width)] transition-all duration-300 p-12 min-w-0 pb-16">
            ...
            <div className="flex items-center justify-between w-full">
              ...
              <div className="flex-1 flex flex-col items-center">
                <h2 className="text-3xl md:text-4xl font-extrabold text-[#256353] tracking-tight text-center mb-2" style={{ letterSpacing: '.01em' }}>
                  Section Mood & Habits Analysis
                </h2>
                <p className="text-lg text-gray-600">
                  Section: <span className="font-bold">{section}</span> | Emotion: <span className="font-bold">{toTitleCase(emotion)}</span>
                </p>
              </div>
              {/* Download PDF Button */}
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              {showSections.map(sec => (
                <PieSection
                  ...
                  isSleepHours={sec.isSleepHours}
                />
              ))}
            </div>
          )}

          {moodType === 'before' && !loading && (
            <motion.div

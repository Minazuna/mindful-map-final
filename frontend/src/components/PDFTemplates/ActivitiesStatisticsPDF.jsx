...
export async function generateActivitiesStatisticsPDF(emotion, moodType, moodPeriod, sectionsData) {
  ...
  try {
    ...
    const periodText = `${moodType === 'before' ? 'Before' : 'After'} Emotion · ${periodDisplay}`;
    ...
    for (let i = 0; i < sectionsData.length; i += 2) {
      ...
      for (let col = 0; col < 2; col++) {
        ...
        section.data.forEach((item, idx) => {
          ...
          const truncatedName = activityName.length > maxNameLength
            ? activityName.substring(0, maxNameLength - 1) + '.'
            : activityName;

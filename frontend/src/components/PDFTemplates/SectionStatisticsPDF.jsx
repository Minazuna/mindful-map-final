...
export async function generateSectionStatisticsPDF(section, emotion, moodType, moodPeriod, sectionsData) {
  ...
  try {
    ...
    const sectionInfo = `Section: ${section} · ${moodType === 'before' ? 'Before' : 'After'} Emotion · ${moodPeriod.charAt(0).toUpperCase() + moodPeriod.slice(1)} Period`;
    ...
    const sectionCamel = section.replace(/\s+/g, '');

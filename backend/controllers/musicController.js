...
exports.getMusicByCategory = async (req, res) => {
  try {
    ...
    console.log('Fetching music for category:', category);
    
    // Try with isActive filter first, then without if no results
    ...
    if (music.length === 0) {
      console.log('No music found with isActive=true for category', category, ', trying without filter');
      music = await Music.find({ 
        category: category.toLowerCase()
      }).sort({ createdAt: -1 });
    }
    
    console.log('Found music for category', category, ':', music.length);
...
exports.getAllMusic = async (req, res) => {
  try {
    ...
    // Try with isActive filter first, then without if no results
    let music = await Music.find({ isActive: true }).sort({ createdAt: -1 });
    
    if (music.length === 0) {
      console.log('No music found with isActive=true, trying without filter');
      music = await Music.find({}).sort({ createdAt: -1 });
    }
    
    console.log('Found music records:', music.length);
...
exports.getMusicCategories = async (req, res) => {
  try {
    // First, let's check all music records for debugging
    const allMusic = await Music.find({});
    console.log('Total music records in DB:', allMusic.length);
    console.log('Sample record:', allMusic[0]);
    
    // Try with and without isActive filter
    const categoriesWithActive = await Music.aggregate([
      { $match: { isActive: true } },
      { $group: { _id: '$category', count: { $sum: 1 } } },
      { $sort: { _id: 1 } }
    ]);
    
    const categoriesWithoutActive = await Music.aggregate([
      { $group: { _id: '$category', count: { $sum: 1 } } },
      { $sort: { _id: 1 } }
    ]);
    
    console.log('Categories with isActive=true:', categoriesWithActive);
    console.log('Categories without isActive filter:', categoriesWithoutActive);
    
    // Use the result that has data, prefer the active filter
    const categories = categoriesWithActive.length > 0 ? categoriesWithActive : categoriesWithoutActive;

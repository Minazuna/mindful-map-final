  ...
  try {
    ...
    if (!hasAnyPairs) {
      ...
      return res.json({
        ...
        thresholds,
        sleep: sleepData,
        message: !anyLogsToday && Object.keys(savedResults).length === 0 ? 'No logs or saved results for this day.' : undefined
      });
    }
...
exports.getHistoricalConcordance = async (req, res) => {
  try {
    ...
    docs.forEach((doc) => {
      ...
      const dateKey = `${dt.getFullYear()}-${String(dt.getMonth() + 1).padStart(2, '0')}-${String(dt.getDate()).padStart(
        2,
        '0'
      )}`;
